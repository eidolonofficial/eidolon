...
4. In setup, inventory the actual project first. Resolve the effective Git hook path with `git rev-parse --path-format=absolute --git-path hooks`, including in worktrees. Never overwrite an existing hook.
